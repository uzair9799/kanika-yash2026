  ...
  return (
    <section className="py-24">
      <div className="container mx-auto max-w-3xl px-4">
        ...
        >
          <div className="mb-12 border-b border-maroon/10 pb-8 text-center">
            <h2 className="font-serif text-3xl uppercase tracking-[2px] text-ink">Lagna Patrika</h2>
            <p className="mt-2 font-serif text-sm italic text-maroon/60">The Sacred Schedule</p>
          </div>
            ...
            {events.map((section, sIndex) => (
              <div key={sIndex} className="day-section">
                <div className="mb-6 border-l-[3px] border-gold pl-4">
                  <span className="font-serif text-lg font-bold text-maroon">{section.date}</span>
                </div>
                ...
                <div className="space-y-4">
                  {section.items.map((item, iIndex) => (
                    <motion.div
                      ...
                      className="flex items-center justify-between border-b border-dotted border-maroon/20 pb-2"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-maroon/40">{item.icon}</span>
                        <span className="font-serif text-sm font-semibold text-ink">{item.title}</span>
                      </div>
                      <span className="font-mono text-xs text-maroon/60">{item.time}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-16 text-center">
            <p className="font-serif text-sm italic text-maroon/60">
